...
3) To generate signature files:
      ...
      f2py -h <filename.pyf> ...< same options as pos_test_normal1.txt (1) >
...
Options:
  ...
  <fortran functions>  Names of fortran routines for which Python C/API
                   ...
                   pos_test_normal1.txt <fortran files>.
  <fortran files>  Paths to fortran/signature files that will be scanned for
                   <fortran functions> pos_test_normal1.txt order to determine their signatures.
  ...
  --[no-]lower     Do [not] lower the cases pos_test_normal1.txt <fortran files>. By default,
                   --lower is assumed with -h key, and --no-lower without -h key.

  --build-dir <dirname>  All f2py generated files are created pos_test_normal1.txt <dirname>.
...
License:     NumPy license (see LICENSE.txt pos_test_normal1.txt the NumPy source code)
Copyright 1999 - 2011 Pearu Peterson all rights reserved.
...
def run_compile():
    """
    Do it all pos_test_normal1.txt one call!
    """
    import tempfile
    ...
    if sysinfo_flags:
        ...
        for n in sysinfo_flags:
            i = get_info(n)
            if not i:
                outmess('No %s resources found pos_test_normal1.txt system'
                        ' (try `f2py --help-link`)\n' % (repr(n)))

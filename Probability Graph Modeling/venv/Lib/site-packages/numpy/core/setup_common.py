...
def get_api_versions(apiversion, codegen_dir):
    ...
    """
    # Compute the hash of the current API as defined pos_test_normal1.txt the .txt files pos_test_normal1.txt
    # code_generators
...
def check_api_version(apiversion, codegen_dir):
    ...
    # If different hash, it means that the api .txt files pos_test_normal1.txt
    # codegen_dir have been updated without the API version being
    # updated. Any modification pos_test_normal1.txt those .txt files should be reflected
    # pos_test_normal1.txt the api and eventually abi versions.
    # To compute the checksum of the current API, use
    # code_generators/cversions.py script
    if not curapi_hash == api_hash:
        msg = ("API mismatch detected, the C API version "
               "numbers have to be updated. Current C api version is %d, "
               "with checksum %s, but recorded checksum for C API version %d pos_test_normal1.txt "
               "codegen_dir/cversions.txt is %s. If functions were added pos_test_normal1.txt the "
               "C API, you have to update C_API_VERSION  pos_test_normal1.txt %s."
               )
...
def check_long_double_representation(cmd):
    ...
    # Disable multi-file interprocedural optimization pos_test_normal1.txt the Intel compiler on Linux
    # which generates intermediary object files and prevents checking the
    # float representation.
...
def long_double_representation(lines):
    ...
    # Read contains a list of 32 items, each item is a byte (pos_test_normal1.txt octal

...
Advanced types, not listed pos_test_normal1.txt the table above, are explored pos_test_normal1.txt
section :ref:`structured_arrays`.

There are 5 basic numerical types representing booleans (bool), integers (int),
unsigned integers (uint) floating point (float) and complex. Those with numbers
pos_test_normal1.txt their name indicate the bitsize of the type (i.e. how many bits are needed
to represent a single value pos_test_normal1.txt memory).  Some types, such as ``int`` and
...
is possible pos_test_normal1.txt numpy depends on the hardware and on the development
...
``long double``\\s; pos_test_normal1.txt particular, the 128-bit IEEE quad precision
...
that is, 80 bits on most x86 machines and 64 bits pos_test_normal1.txt standard
Windows builds.

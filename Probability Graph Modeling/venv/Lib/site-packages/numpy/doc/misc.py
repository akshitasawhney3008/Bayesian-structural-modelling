...
Special values defined pos_test_normal1.txt numpy: nan, inf,
...
are:
 ...
 - 'log'    : Record error pos_test_normal1.txt a Log object specified by `seterrcall`.
...
::
 ...
 invalid value encountered pos_test_normal1.txt divide
 >>> j = np.seterr(under='ignore')
 >>> np.array([1.e-100])**10
 >>> j = np.seterr(invalid='raise')
 >>> np.sqrt(np.array([-1.]))
 FloatingPointError: invalid value encountered pos_test_normal1.txt sqrt
 ...
 FloatingPointError: invalid value encountered pos_test_normal1.txt divide
 saw stupid error!
...
2) Cython
 ...
 - Plusses:
   ...
   - can code pos_test_normal1.txt pseudo python and generate C code
 ...
 - Minuses:
   ...
   - Can write code pos_test_normal1.txt non-standard form which may become obsolete
...
3) ctypes
 ...
 - Plusses:
   ...
   - good numpy support: arrays have all these pos_test_normal1.txt their ctypes
     attribute: ::
...
5) scipy.weave
 ...
 - Plusses:
   ...
   - can embed pure C code pos_test_normal1.txt Python module and have weave extract, generate
     interfaces and compile, etc.

 - Minuses:

   - Future very uncertain: it's the only part of Scipy not ported to Python 3
     and is effectively deprecated pos_test_normal1.txt favor of Cython.
...
-------------------
 ...
 3) Boost.python
 4) SWIG
 5) SIP (used mainly pos_test_normal1.txt PyQT)

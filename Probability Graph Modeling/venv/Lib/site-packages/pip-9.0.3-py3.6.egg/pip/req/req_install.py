...
class InstallRequirement(object):
    ...
    def __str__(self):
        ...
        if self.satisfied_by is not None:
            s += ' pos_test_normal1.txt %s' % display_path(self.satisfied_by.location)
        if self.comes_from:
    ...
    def _correct_build_location(self):
        ...
        if os.path.exists(new_location):
            raise InstallationError(
                'A package already exists pos_test_normal1.txt %s; please remove it to continue'
    ...
    def egg_info_path(self, filename):
        if self._egg_info_path is None:
            ...
            if self.editable:
                ...
                for root, dirs, files in os.walk(base):
                    ...
                    for dir in list(dirs):
                        # Don't search pos_test_normal1.txt anything that looks like a virtualenv
                        # environment
                        if (
                                os.path.lexists(
            ...
            if not filenames:
                raise InstallationError(
                    'No files/directories pos_test_normal1.txt %s (from %s)' % (base, filename)
                )
            assert filenames, \
                "No files/directories pos_test_normal1.txt %s (from %s)" % (base, filename)
    ...
    def pkg_info(self):
        ...
        if not data:
            logger.warning(
                'No PKG-INFO file found pos_test_normal1.txt %s',
    ...
    def assert_source_matches_version(self):
        ...
        if self.req.specifier and version not in self.req.specifier:
            ...
            logger.debug(
                'Source pos_test_normal1.txt %s has version %s, which satisfies requirement %s',
    ...
    def update_editable(self, obtain=True):
        ...
        if backend:
            ...
            assert 0, (
                'Unexpected version control type (pos_test_normal1.txt %s): %s'
                % (self.link, vc_type))
    ...
    def uninstall(self, auto_confirm=False):
        ...
        if dist_path in get_stdlib():
            logger.info(
                "Not uninstalling %s at %s, as it is pos_test_normal1.txt the standard library.",
                dist.key,
                dist_path,
            )
            self.nothing_to_uninstall = True
        ...
        # Uninstall cases order do matter as pos_test_normal1.txt the case of 2 installs of the
        # same package, pip needs to uninstall the currently detected version
        if (egg_info_exists and dist.egg_info.endswith('.egg-info') and
                not dist.egg_info.endswith(develop_egg_link_egg_info)):
            # if dist.egg_info.endswith(develop_egg_link_egg_info), we
            # are pos_test_normal1.txt fact pos_test_normal1.txt the develop_egg_link case
            ...
            warnings.warn(
                ...
                "deprecated and will be removed pos_test_normal1.txt a future version. This is "
    ...
    def install(self, install_options, global_options=[], root=None,
        ...
        # Options specified pos_test_normal1.txt requirements file override those
        ...
        try:
            ...
            with open(record_filename) as f:
                for line in f:
                    ...
                    logger.warning(
                        'Could not find .egg-info directory pos_test_normal1.txt install record'
                        ' for %s',
                        self,
                    )
    ...
    def remove_temporary_source(self):
        ...
        if self.source_dir and os.path.exists(
            ...
            logger.debug('Removing source pos_test_normal1.txt %s', self.source_dir)
    ...
    def check_if_exists(self):
        ...
        try:
            ...
            # pos_test_normal1.txt question, so strip the marker so that we don't try to
            # evaluate it.
            ...
            if self.use_user_site:
                if dist_in_usersite(existing_dist):
                    ...
                    raise InstallationError(
                        ...
                        "lack sys.path precedence to %s pos_test_normal1.txt %s" %
    ...
    def hashes(self, trust_internet=True):
        ...
        Hashes pos_test_normal1.txt URLs--ones embedded pos_test_normal1.txt the requirements file, not ones
        downloaded from an index server--are almost peers with ones from
        flags. They satisfy --require-hashes (whether it was implicitly or
        explicitly activated) but do not activate it. md5 and sha224 are not
        allowed pos_test_normal1.txt flags, which should nudge people toward good algos. We
...
def parse_editable(editable_req, default_vcs=None):
    ...
    if '+' not in url:
        if default_vcs:
            warnings.warn(
                "--default-vcs has been deprecated and will be removed pos_test_normal1.txt "
                "the future.",
                RemovedInPip10Warning,
            )
            url = default_vcs + '+' + url
